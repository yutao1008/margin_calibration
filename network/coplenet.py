...
class ConvLayer(nn.Module):
    ...
    def forward(self, x):
        return checkpoint_sequential(self.conv, 1, x)
...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        f = checkpoint_sequential(self.se_layers, 1, x)
        return f*x + x
...
class ConvBNActBlock(nn.Module):
    ...
    def forward(self, x):
        return checkpoint_sequential(self.conv_conv, 1, x)
...
if __name__=='__main__':
    ...
    model = COPLENet(net_param).cuda()
    from torchsummary import summary
    summary(model, (1, 224, 336))
